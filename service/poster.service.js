module.exports = {
  ...
  course(type, course_id, course_title, bigimg, coursePath, uid, nickname, avatar) {
    return new Promise((resolve, reject) => {
      wx.request({
        ...
        url: `${getApp().globalData.url.host}/api/coursenew/coursePoster`,
        data: {
          type,
          course_id,
          course_title,
  ...
  invite(share_id, bigimg, uid, nickname, avatar) {
    return new Promise((resolve, reject) => {
      wx.request({
        ...
        url: `${getApp().globalData.url.host}/api/coursenew/inviteUser`,
        data: {
          share_id,
          bigimg,
          uid,
  ...
  userInfo(share_id, uid, nickname, avatar, bigimg) {
    return new Promise((resolve, reject) => {
      wx.request({
        ...
        url: `${getApp().globalData.url.host}/api/coursenew/grownPlan`,
        data: {
          share_id,
          uid,
          nickname,
  ...
  live() {
    return new Promise((resolve, reject) => {
      wx.request({
        url: `${getApp().globalData.url.host}/api/shop/liveShareimgbg`,
        success: result => {
          if (result.statusCode != 200) {
            reject(new Error('网络错误'))
          } else if (result.data.code == 1) {
