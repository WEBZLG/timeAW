...
Page({
      ...
      success: async res => {
        if (res.confirm) {
          ...
          try {
            ...
            // 获取数据
            const result = await SEARCH_SERVICE.clearHistory(wx.getStorageSync('userInfo').id)
  ...
  async init() {
    ...
    try {
      ...
      // 获取数据
      const result = await SEARCH_SERVICE.infomation(wx.getStorageSync('userInfo').id)
